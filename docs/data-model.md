...
Согласно ADR 003, выбранный транспорт — SSE. Сервер присваивает событиям
глобальные монотонные `sequence` и уникальные `eventId`, сохраняет ограниченную
историю и поддерживает
replay после `Last-Event-ID`. Клиент применяет только следующий contiguous
sequence; дубликаты и старые события игнорируются. При gap клиент делает полный
GET как baseline recovery snapshot и возобновляет поток после последнего
успешно применённого event id, чтобы replay доставил пропущенные события. Это
необходимо, поскольку mock server не персистит сгенерированные metric patches.
Если recovery GET не удался, текущая точка сохраняется, а поток
переподключается с начала новой stream-позиции.

SSE используется вместо буквального WebSocket из задания, поскольку dashboard
нужен только server-to-client поток metric-only patches. Семантика патча остаётся
той же: identity и topology не меняются, изменяться могут только `headcount`,
`budget` и `performance`.
