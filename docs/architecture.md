...
Согласно ADR 003, SSE-ресурс принадлежит dashboard resource и закрывается при
уходе последнего потребителя. Сервер хранит bounded history последних событий,
принимает
`Last-Event-ID` и эквивалентный query-параметр для управляемого client restart,
а затем replay-ит доступные события после указанного id. Медленный клиент
отключается при отказе записи, чтобы не создавать неограниченный буфер.

При gap в sequence клиент временно прекращает применение патчей и выполняет
полный GET как baseline recovery snapshot. После этого SSE возобновляется с
последнего успешно применённого event id, чтобы bounded-history replay доставил
все события, выявившие gap; recovery GET сам по себе не считается источником
realtime-патчей, потому что mock server не персистит сгенерированные изменения.
Timestamp последнего патча участвует в structural sharing, поэтому более
старый полный GET не затирает более новое realtime-значение.
